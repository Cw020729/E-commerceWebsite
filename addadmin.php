...
<?php
    ...
    if(isset($_POST['UserName'])){
        ...
        if($valid == ""){
            ...
            $check1 = $stmt->fetchAll();
            if((count($check) == 0 ) && (count($check1)==0) && ($_POST['Password'] == $_POST['Reenterpassword'])){   
                $query = "INSERT INTO admin_info (AdminID,First_name,Last_name,Username,Dob,Gender,Email,Phone,Pwd) VALUES(NULL,$firstname,$lastname,$myusername,$dob,'$gender',$email,$phone,'$myhashpassword')";
...
<html>
    ...
    <body>
        <div class="signupbox">
            <div style="font-size:30px;text-align:center;padding-bottom:15px;">Admin</div>
            <div style="text-align:center; color:grey; padding-bottom:15px;" >Fill in all the blank required to add an admin account.</div>
            <hr>
                <form method="post" name="Admin" onsubmit="return NoEmptyForm()">
